...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'consumption_daily') THEN
        ...
        SELECT DISTINCT
            ...
            EXTRACT(ISODOW FROM date)::INTEGER - 1,
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_available_extensions WHERE name = 'pg_trgm') THEN
        ...
        EXCEPTION WHEN others THEN
            RAISE NOTICE 'Impossible de créer l''extension pg_trgm (droits insuffisants?) - saut du trigram index';
        END;
        IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_trgm') THEN
            EXECUTE 'CREATE INDEX IF NOT EXISTS idx_dim_users_telephone_trgm ON dim_users USING gin(telephone gin_trgm_ops)';
        ELSE
            RAISE NOTICE 'pg_trgm non installé : saut du trigram index';
        END IF;
    ELSE
        RAISE NOTICE 'pg_trgm non disponible sur cette instance : saut du trigram index';
    END IF;
END
$$;
